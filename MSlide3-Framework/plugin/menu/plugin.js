...
const Plugin = () => {
  ...
  let slideMenuFooterLinks = [
    {
      "title": "Terms & Conditions",
      "url": "https://blstdemo.website/mslide/mslide1.6.6/master1.6.6.html"
    },
    {
      "title": "Cookie policy",
      "url": "https://blstdemo.website/mslide/mslide1.6.6/master1.6.6.html"
  ...
  window.onload = async function () {
    ...
    async function generateThumbails() {
      // Check device width and orientation
      if ((window.innerWidth <= 881 && window.matchMedia("(orientation: landscape)").matches === true) || window.innerWidth < 481) {
        return;
      }
    
      // Get elements
      const div = document.querySelector('div[data-panel="Slides"]');
      const sections = selectAll('.slides > section');
      const menuImages = []; // Array to store image paths
    
      // Loop through sections and subsections
      for (let h = 0; h < sections.length; h++) {
        const subsections = selectAll('section', sections[h]);
    
        for (let v = 0; v < subsections.length; v++) {
          const subsection = subsections[v];
    
          // Construct image path
          const imagePath = `images/thumbnails/section-${h}-${v}.png`;
    
          // Store image path
          menuImages.push(imagePath);
    
          // Set data-menu-img attribute
      ...
      if (div) {
        const ulList = div.querySelectorAll('ul.slide-menu-items');
    
        if (ulList.length > 0) {
          ulList.forEach((ul) => {
            const liList = ul.querySelectorAll('li.slide-menu-item, li.slide-menu-item-vertical');
    
            // Loop through li elements and set image src
            for (let i = 0; i < liList.length && i < menuImages.length; i++) {
              const li = liList[i];
    ...
    // await generateThumbails();

  async function createMenuFooter() {
    // Select the existing div element
    var divElement = document.querySelector('div[data-panel="Slides"]');

    // create a div with slide-menu-footer-wrapper class
    var footerWrapper = document.createElement('div');
    footerWrapper.classList.add('slide-menu-footer-wrapper');

    // Append the footerWrapper to the divElement
    divElement.appendChild(footerWrapper);
    ...
    for (var i = 0; i < slideMenuFooterLinks.length; i++) {
    ...
    var existingUl = divElement.querySelector('.slide-menu-items');
    ...
    if (!pwaInstalled) {
      // if ((window.innerWidth <= 881 && window.matchMedia("(orientation: landscape)").matches === true) || window.innerWidth < 481) {
          ...
          var slideDownloadButton = document.createElement('button');
          // add fa fa-download icon and Download text
          ...
          slideDownloadButton.addEventListener('click', async function() {
          ...
          newLi.appendChild(slideDownloadButton);
          newUl.appendChild(newLi);
    ...
    // add the new ul to the footerWrapper
    footerWrapper.appendChild(newUl);

    
  }

    await createMenuFooter();
}
...
function delay(ms) {
  ...
  function initMenu() {
    if (!initialised) {
      ...
      button.appendChild(create('i', { class: 'iconoir-xmark' }));
      ...
      function generateItem(type, section, i, h, v) {
        ...
        if (img !== null){
          ...
          const canvas = create('img', { class: 'slide-menu-item-img' })
          item.appendChild(canvas);
          const content = select('.reveal').querySelector('.reveal')
        ...
        if (options.markers) {
          item.appendChild(
            create('i', { class: 'fas iconoir-check-circle-solid fa-fw past' })
          );
          item.appendChild(
            create('i', {
          ...
          item.appendChild(
            create('i', { class: 'far iconoir-circle fa-fw future' })
          );
        }
      ...
      function createSlideMenu() {
        if (
          ...
          if (window.innerWidth > 2560) {
            ...
            selectAll('.reveal > .slides > .scroll-page > .scroll-page-sticky > .scroll-page-content > section').forEach(function (section, h) {
              // from every section, extract the data-index-h and data-index-v
              var h = section.getAttribute('data-index-h');
              var v = section.getAttribute('data-index-v');
      ...
      if (options.openButton) {
        ...
        link.appendChild(create('i', { class: 'iconoir-menu' }));
