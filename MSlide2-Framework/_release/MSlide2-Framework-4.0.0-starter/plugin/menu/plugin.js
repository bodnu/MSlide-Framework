    ...
    async function generateThumbails() {
      ...
      for (let h = 0; h < sections.length; h++) {
        ...
        for (let v = 0; v < subsections.length; v++) {
          ...
          const customThumb = subsection.getAttribute('data-menu-img');
          const imagePath = customThumb && customThumb.trim() ? customThumb : `assets/thumbnails/section-${h}-${v}.png`;
  ...
  function initMenu() {
    if (!initialised) {
      ...
      function generateItem(type, section, i, h, v) {
        ...
        if (!img || !img.trim()) {
          img = `assets/thumbnails/section-${h}-${v === undefined ? 0 : v}.png`;
        }
        if (img){
      ...
      if (options.openButton) {
        ...
        link.appendChild(create('i', { class: 'iconoir-multiple-pages-empty' }));
